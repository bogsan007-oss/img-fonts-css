console.log("Мотор плеера с радио и большим визуализатором загружен");
...
// ВАЖНО: переменная должна быть объявлена!
...
function animateVisualizer() {
    ...
    // === НИЧЕГО НЕ ИГРАЕТ → ГАСИМ ВИЗУАЛИЗАТОР ===
    if (!isPlaying) {
        bars.forEach(bar => bar.style.transform = "scaleY(0)");
        return;
    }
